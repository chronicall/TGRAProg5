...
public class Shader {
	private static int RING_LIGHTS = 5;
	...
	private int ringLightLoc[][];
	private int ringLightPositionsLoc[];
	...
	public Shader(String vertexShader, String fragmentShader) {
		...
		this.ringLightLoc = new int[Shader.RING_LIGHTS][3];
		for (int i = 0; i < Shader.RING_LIGHTS; i++) {
			this.ringLightLoc[i][0] = Gdx.gl.glGetUniformLocation(this.renderingProgramID, "u_ringLights[" + i + "].position");
			this.ringLightLoc[i][1] = Gdx.gl.glGetUniformLocation(this.renderingProgramID, "u_ringLights[" + i + "].attenuation");
			this.ringLightLoc[i][2] = Gdx.gl.glGetUniformLocation(this.renderingProgramID, "u_ringLights[" + i + "].colour");
		}
		this.ringLightPositionsLoc = new int[Shader.RING_LIGHTS];
		for (int i = 0; i < Shader.RING_LIGHTS; i++) {
			this.ringLightPositionsLoc[i] = Gdx.gl.glGetUniformLocation(this.renderingProgramID, "u_ringLightPositions[" + i + "]");
		}
		this.shineDamperLoc				= Gdx.gl.glGetUniformLocation(this.renderingProgramID, "u_shineDamper");
		this.reflectivityLoc			= Gdx.gl.glGetUniformLocation(this.renderingProgramID, "u_reflectivity");
...
//			Gdx.gl.glTexParameteri(GL20.GL_TEXTURE_2D, GL20.GL_TEXTURE_MIN_FILTER, GL20.GL_LINEAR_MIPMAP_LINEAR);
	...
	public void setRingLights(ArrayList<SpotLight> ringLights) {
		int i = 0;
		for (SpotLight light : ringLights) {
			Gdx.gl.glUniform4f(this.ringLightPositionsLoc[i], light.getPosition().x, light.getPosition().y, light.getPosition().z, 1.0f);
			Gdx.gl.glUniform4f(this.ringLightLoc[i][0], light.getPosition().x, light.getPosition().y, light.getPosition().z, 1.0f);
			Gdx.gl.glUniform3f(this.ringLightLoc[i][1], light.getAttenuation().x, light.getAttenuation().y, light.getAttenuation().z);
			Gdx.gl.glUniform4f(this.ringLightLoc[i][2], light.getColour().r, light.getColour().g, light.getColour().b, light.getColour().a);
			i++;
		}
	}
	
	public void setShineDamper(float shineDamper) {
		Gdx.gl.glUniform1f(this.shineDamperLoc, shineDamper);
	}
	public void setReflectivity(float reflectivity) {
		Gdx.gl.glUniform1f(this.reflectivityLoc, reflectivity);
	}
}
